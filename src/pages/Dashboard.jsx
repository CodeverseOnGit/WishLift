...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      isRecipient
        ? wishesService.getUserWishes(user.id)
        : wishesService.getSavedWishes(user.id),
      messagesService.getContactRequests(user.id, profile.role),
    ])
      .then(([w, cr]) => {
        if (isRecipient) setWishes(w);
        else setSavedWishes(w);
        ...
        <div className="dashboard-header">
          <div>
            ...
            <p className="page-subtitle">
              {isRecipient
                ? "Manage your wishes and contact requests"
                : "Track the wishes you're supporting"}
            </p>
          </div>
          {isRecipient && (
            <Link to="/create-wish" className="btn btn-primary">
              + Post a Wish
            </Link>
          )}
        </div>
          ...
          {isRecipient ? (
            <>
              ...
              <StatCard
                icon="✅"
                label="Fulfilled"
                value={wishes.filter((w) => w.status === "Fulfilled").length}
                color="green"
              />
              <StatCard
                icon="🔄"
                label="In Progress"
                value={wishes.filter((w) => w.status === "In Progress").length}
                color="gray"
              />
            </>
          ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {(isRecipient ? wishes : savedWishes).length === 0 ? (
              <div className="empty-state">
                ...
                <p>
                  {isRecipient
                    ? "Post your first wish and start connecting with helpers."
                    : "Browse wishes and save the ones you want to help with."}
                </p>
                <Link
                  to={isRecipient ? "/create-wish" : "/browse"}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {contactRequests.length === 0 ? (
              <div className="empty-state">
                ...
                <p>
                  {isRecipient
                    ? "When helpers reach out, their requests appear here."
                    : "Your contact requests to wish owners appear here."}
                </p>
              </div>
            ) : (
                  ...
                  <div key={req.id} className="request-card card">
                    <div className="request-wish-info">
                      {req.wishes?.image_url && (
                        <img
                          src={req.wishes.image_url}
                          alt={req.wishes?.title}
                          className="request-wish-img"
                        />
                      )}
                      <div>
                        <Link
                          to={`/wish/${req.wish_id}`}
                          className="request-wish-title"
                        >
                          {req.wishes?.title}
                        </Link>
                    ...
                    <div className="request-card-footer">
                      <span
                        className={`badge ${req.status === "Pending" ? "badge-yellow" : req.status === "Accepted" ? "badge-green" : "badge-gray"}`}
                      >
                        {req.status}
                      </span>
